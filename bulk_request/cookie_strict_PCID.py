...
logging.info("Using fixed cookies:")
for key, value in session.cookies.items():
    logging.info(f"  {key}: {value}")

# PCID 값 저장 (고정값 사용)
pcid = session.cookies.get('PCID', 'PCID not found')
logging.info(f"Using fixed PCID: {pcid}")
...
for i in range(num_requests):
    try:
        ...
        response = session.get(url, headers=headers, timeout=10)  # 10초 타임아웃 설정
        end_time = time.time()
